...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const batchResult = await db.query(`SELECT id, quantity_generated, quantity_redeemed FROM batches ORDER BY id ASC`, []);
    const batches = batchResult.rows.map((row: any) => ({ id: row.id, quantity_generated: row.quantity_generated, quantity_redeemed: row.quantity_redeemed }));
